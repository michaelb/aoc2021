...
fn most_common_bit_at(values: &Vec<Vec<u32>>, position: u32) -> u32 {
    ...
    let gamma: Vec<_> = acc
        .iter()
        .map(|x| match x * 2 > values.len() as u32 {
            true => 1,
            false => 0,
        })
        .collect();
    return gamma[position as usize];
}
...
fn part2() {
    ...
    for i in 0..12 {
        ...
        let values:Vec<Vec<u32>> = values.clone().into_iter().filter(|v| v[i as usize] == bit).collect();
        if values.len() <= 1 {
    ...
    for i in 0..12 {
        ...
        let bit = 1 - most_common_bit_at(&values, i);
        let values:Vec<Vec<u32>> = values.clone().into_iter().filter(|v| v[i as usize] == bit).collect();
        if values.len() <= 1 {
    ...
    println!("ox: {}, co2: {}, product: {}",ox_rating, co2_rating, ox_rating*co2_rating);
